...
class Versionuploadlist extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      visible: false, // 详情窗口
  ...
  render() {
    const { codeList, loading, enumList, imglist, getImageList, columns,
      onAddVersion } = this.props;
      ...
      <div className="outline-out">
        <div className="outline-inner">
          <div className="outline-inner-padding">
            <Row>
              <Col span={14}>
                <div>搜索结果 共<span className="allNumLight">{resultlist ? resultlist.length : 0}</span> 条</div>
              </Col>
              <Col span={8} style={{ textAlign: 'right' }}>
                <Button type="primary" style={{ margin: '0 5px' }} onClick={onAddVersion}>新增</Button>
              </Col>
            </Row>
          </div>
          <Table
...
export default Form.create()(Versionuploadlist);
