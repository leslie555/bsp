...
import total1 from 'assets/settingdrawer/total1.svg';
import total2 from 'assets/settingdrawer/total2.svg';
...
class SettingDarwer extends PureComponent {
  componentDidMount() {
    const {
      setting: { themeColor, colorWeak, hideSwitchMainpage },
    } = this.props;
  ...
  render() {
    const { setting } = this.props;
    const { collapse, silderTheme, themeColor, layout, colorWeak, hideSwitchMainpage } = setting;
    return (
      <Drawer
        firstEnter
        visible={collapse}
            ...
            <List.Item
              actions={[
                <Switch
                  ...
                  checkedChildren="开"
                  unCheckedChildren="关"
                  checked={!!hideSwitchMainpage}
                  onChange={checked => this.changeSetting('hideSwitchMainpage', checked)}
                />,
              ]}
            >
              隐藏切换工作台按钮
