...
export class A18 extends React.Component {
  componentDidMount() {

  }

  render() {
    const { bigStyle, monthAddNum } = this.props;
    const num = monthAddNum === null ? 0 : monthAddNum;
    return (
      <div>
        <Card className={styles.cardStyle} style={bigStyle === 'big' ? { height: '142px' } : { height: '124px' }}>
          <div style={{ display: 'inline-block', textAlign: 'center', lineHeight: '40px', width: bigStyle === 'big' ? '34%' : '40%', marginTop: '6%', zIndex: '2', position: 'relative' }}>
            <img alt="example" src={require('../../assets/新增申请.png')} style={{ width: '40%' }} />
            <div style={{ position: 'relative', color: '#fff' }}>本月新增申请数</div>
          </div>
...
export default connect(state => ({
  monthAddNum: state.chart.monthAddNum,
}))(A18);
