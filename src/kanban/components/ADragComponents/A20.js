...
/* eslint-disable   */
export class A20 extends React.Component {


  componentDidMount() {
    if (this.props.digroadNum === null) {
  ...
  render() {
    ...
    const num = this.props.digroadNum === null ? 0 : this.props.digroadNum;
    return (
      <div>
        <Card className={styles.cardStyle} style={bigStyle === 'big' ? { height: '142px' } : { height: '124px' }}>
          <div style={{ display: 'inline-block', textAlign: 'center', lineHeight: '40px', width: bigStyle === 'big' ? '34%' : '40%', marginTop: '6%', zIndex: '2', position: 'relative' }}>
            <img alt="example" src={require('../../assets/正在施工.png')} style={{ width: '40%' }} />
            <div style={{ position: 'relative', color: '#fff' }}>施工中数</div>
          </div>
...
export default connect(state => ({
  digroadNum: state.chart.digroadNum,
}))(A20);
