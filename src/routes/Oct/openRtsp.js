...
class openRtsp extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      visible: true,
      ...
      <div style={{ height: '300px', width: '600px', position: 'absolute', left: 'calc(50% - 300px)', top: '50px', padding: '20px', border: '1px solid rgba(0,0,0,0.2)' }}>
        <Row>
          ...
          <Col span={24} style={{ width: '100%', paddingTop: '0', fontSize: '18px' }}>
            ...
            <div align="center">
              <Button onClick={() => this.openExe('file:///D:/monitor/bin/Monitor.exe')} style={{ textAlign: 'center' }}>打开Rtsp</Button>
            </div>
          </Col>
        </Row>
      </div>
...
function mapStateToProps({ octopenRtsp, common, loading }) {
  return { octopenRtsp, common, loading: loading.models.octopenRtsp };
}
export default connect(mapStateToProps)(openRtsp);
