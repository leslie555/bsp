...
import Search from '../../components/oct/DeviceEventShieldSearch';
import List from '../../components/oct/DeviceEventShieldList';

const { confirm } = Modal;

class DeviceEventShield extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      selectedKeys: [],
  ...
  render() {
    const { octDeviceEventShield, common, dispatch } = this.props;
    const {
      queryobj,
      pagination,
      resultlist,
    } = octDeviceEventShield;
    ...
    const searchprops = {
      ...
      onSearch: (query) => {
        ...
        dispatch({
          type: 'octDeviceEventShield/queryDevice',
...
function mapStateToProps({ octDeviceEventShield, common }) {
  return { octDeviceEventShield, common };
}
export default connect(mapStateToProps)(DeviceEventShield);
