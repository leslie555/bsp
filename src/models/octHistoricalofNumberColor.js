...
import { queryGetTreeOnlyLeaf, queryHistoricalofNumberColor } from '../services/octCollectionReportService';

export default {
  namespace: 'octHistoricalofNumberColor',
  state: {
    treeData: [],
    departmentlist: [],
    departmentinfo: {},
  ...
  effects: {
    ...
    *queryMenuList({ payload }, { call, put }) {
    ...
    // 查询号牌分类历史记录
    *queryHistoricalofNumberColor({ payload }, { call, put }) {
      const { pagination = {} } = payload;
      const { queryobj = {} } = payload;
      const obj = { ...queryobj, pagination };
      const data = yield call(queryHistoricalofNumberColor, obj);
      console.log(data);
      if (data.result) {
        yield put({
          type: 'updateState',
