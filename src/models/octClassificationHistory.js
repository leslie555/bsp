...
import { queryGetTreeOnlyLeaf, queryClassificationHistory } from '../services/octCollectionReportService';

export default {
  namespace: 'octClassificationHistory',
  state: {
    treeData: [],
    departmentlist: [],
    departmentinfo: {},
  ...
  effects: {
    ...
    *queryMenuList({ payload }, { call, put }) {
    ...
    // 车籍分类历史记录
    *queryClassificationHistory({ payload }, { call, put }) {
      const { pagination = {} } = payload;
      const { queryobj = {} } = payload;
      const obj = { ...queryobj, pagination };
      const data = yield call(queryClassificationHistory, obj);
      if (data.result) {
        yield put({
          type: 'updateState',
          payload: {
